...
import { filterRepositoriesWithMoreThanFiveStars, getLastFiveUpdatedRepositories, sumOfAllRepositoryStars }  from '../src/repositoryUtils.js';
import { mockedRepositories, mockedRepositoriesFull, mockedRepositoriesLess }  from './mockedData.js';
...
describe('filterRepositoriesWithMoreThanFiveStars', () => {
    describe('when it receives an array of objects', () => {
        it('should return repositories with more than 5 stars', () => {
            const filteredRepositories = filterRepositoriesWithMoreThanFiveStars(mockedRepositories);
...
describe('getLastFiveUpdatedRepositories', () => {
    describe('when it receives an array of objects', () => {
        it('should return the last 5 updated repositories', () => {
            const lastFiveUpdatedRepositories = getLastFiveUpdatedRepositories(mockedRepositoriesFull);

            expect(lastFiveUpdatedRepositories).to.deep.equal([
                { id: 6, name: 'repo6', updated_at: '2023-04-06T12:00:00Z', stargazers_count: 5 },
                { id: 5, name: 'repo5', updated_at: '2023-04-05T12:00:00Z', stargazers_count: 5 },
                { id: 4, name: 'repo4', updated_at: '2023-04-04T12:00:00Z', stargazers_count: 5 },
                { id: 3, name: 'repo3', updated_at: '2023-04-03T12:00:00Z', stargazers_count: 5 },
                { id: 2, name: 'repo2', updated_at: '2023-04-02T12:00:00Z', stargazers_count: 5 }
            ]);
        });

        it('should return all repositories if there are less than 5', () => {
            const lastFiveUpdatedRepositories = getLastFiveUpdatedRepositories(mockedRepositoriesLess);
